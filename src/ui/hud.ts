...
export class HUD {
  ...
  private bindDom(): void {
    ...
    this.btnToggleTrainConfigEl = document.getElementById(
      'btn-toggle-train-config'
    )!
    this.btnCloseTrainConfigEl = document.getElementById(
      'btn-close-train-config'
    )!

    this.worldConfigCardEl = document.getElementById('world-config-card')!
    this.btnToggleWorldConfigEl = document.getElementById(
      'btn-toggle-world-config'
    )!
    this.btnCloseWorldConfigEl = document.getElementById(
    ...
    this.btnToggleAdvancedConfigEl = document.getElementById(
      'btn-toggle-advanced-config'
    )!
    this.btnCloseAdvancedConfigEl = document.getElementById(
      'btn-close-advanced-config'
    )!
  ...
  private setupEventListeners(): void {
    ...
    document
      .getElementById('btn-top-controls')
      ?.addEventListener('click', () => {
        this.openControlsModal()
      })

    document
      .getElementById('btn-respawn-train')
      ?.addEventListener('click', () => {
        this.respawnTrain()
      })
    ...
    this.btnToggleAdvancedEl?.addEventListener('click', () => {
      ...
      if (next) {
        this.triggerAlert({
          ...
          message:
            'Deadman control system enabled. Acknowledge with Q or cab pedal.',
          canReset: true,
        })
    ...
    const setShape = (shape: WorldShape) => {
      ...
      if (shape === 'I')
        document.getElementById('btn-shape-i')?.classList.add('active')
      if (shape === 'S')
        document.getElementById('btn-shape-s')?.classList.add('active')
      if (shape === 'O')
        document.getElementById('btn-shape-o')?.classList.add('active')
    }

    document
      .getElementById('btn-shape-i')
      ?.addEventListener('click', () => setShape('I'))
    document
      .getElementById('btn-shape-s')
      ?.addEventListener('click', () => setShape('S'))
    document
      .getElementById('btn-shape-o')
      ?.addEventListener('click', () => setShape('O'))
    ...
    const setSize = (size: WorldSize) => {
      ...
      if (size === 'S')
        document.getElementById('btn-size-s')?.classList.add('active')
      if (size === 'M')
        document.getElementById('btn-size-m')?.classList.add('active')
      if (size === 'L')
    ...
    document
      .getElementById('btn-size-s')
      ?.addEventListener('click', () => setSize('S'))
    document
      .getElementById('btn-size-m')
      ?.addEventListener('click', () => setSize('M'))
    document
      .getElementById('btn-size-l')
      ?.addEventListener('click', () => setSize('L'))
    document
    ...
    document
      .getElementById('btn-type-regional')
      ?.addEventListener('click', () => setType('regional'))
    document
      .getElementById('btn-type-cargo')
      ?.addEventListener('click', () => setType('cargo'))
    document
      .getElementById('btn-type-highspeed')
      ?.addEventListener('click', () => setType('high_speed'))
    ...
    document
      .getElementById('btn-invert-train')
      ?.addEventListener('click', () => {
    ...
    window.addEventListener('keydown', e => {
      ...
      if (!isRepeat) {
        ...
        if (e.code === 'KeyW' || e.code === 'ArrowUp') {
          this.train.targetThrottle = clamp(
            +(this.train.targetThrottle + 0.01).toFixed(2),
            0,
            1
          )
          ...
          this.train.targetBrake = clamp(
            +(this.train.targetBrake + 0.01).toFixed(2),
            0,
            1
          )
        } else if (e.code === 'KeyV') {
    ...
    setInterval(() => {
      ...
      if (keysPressed['KeyW'] || keysPressed['ArrowUp']) {
        this.train.targetThrottle = clamp(
          +(this.train.targetThrottle + fineStep).toFixed(2),
          0,
          1
        )
      }
      ...
      if (keysPressed['Space'] || keysPressed['KeyB']) {
        this.train.targetBrake = clamp(
          +(this.train.targetBrake + fineStep).toFixed(2),
          0,
          1
        )
      }
  ...
  public acknowledgeDeadman(): void {
    ...
    if (acked) {
      if (
        this.activeAlert &&
        (this.activeAlert.title.toLowerCase().includes('deadman') ||
          this.activeAlert.message.toLowerCase().includes('deadman'))
      ) {
        if (!this.train.isEmergencyBrakeLocked) {
          this.clearAlert()
        }
  ...
  public syncAdvancedConfigUI(): void {
    ...
    if (this.btnToggleDeadmanEl) {
      ...
      ;(this.btnToggleDeadmanEl as HTMLButtonElement).disabled =
        !cfg.advancedControls
    }
  ...
  public setReverser(pos: 1 | 0 | -1): void {
    ...
    if (
      ...
      (this.train.speedKmH !== 0 || Math.abs(this.train.speed) > 0.05)
    ) {
      this.triggerAlert({
        type: 'info',
  ...
  public updateLeverHandles(): void {
    ...
    if (this.reverserValEl)
      this.reverserValEl.textContent =
        rev === 1 ? 'FWD' : rev === -1 ? 'REV' : 'NEU'
  ...
  public update(stationStatus: StationStatus): void {
    ...
    if (this.activeAlert) {
      if (this.train.isCrashed) {
        this.warningBannerEl.classList.remove('info-alert')
        this.warningBadgeEl.innerHTML =
          '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>'
        if (this.train.isDerailed) {
          this.warningTitleEl.textContent = 'Train derailed'
          this.warningDescEl.textContent =
            this.train.crashReason ||
            'Excessive lateral speed through curve. Respawn required.'
        } else {
        ...
        if (speed === 0) {
          this.warningDescEl.textContent =
            'Train stopped. Press Space or click Reset to restore controls.'
          this.warningResetBtnEl.classList.remove('hidden')
        } else {
    ...
    const nextStn = this.stationMgr.getNextStationAhead(
      this.train.trackId,
      this.train.distance,
      currentTrackLen
    )
    ...
    if (nextSig) {
      ...
      if (nextSig.signal.aspect === 'slow') {
        this.signalDotEl.style.background =
          'linear-gradient(180deg, #10b981 50%, #f59e0b 50%)'
