...
export class DeadmanControlSystem {
  ...
  public update(dt: number, speedKmH: number, isEmergencyBrakeLocked: boolean): DeadmanEvent | null {
    if (!this.enabled) {
      this.timer = 0;
      this.stage = 'none';
  ...
  public getStatus(speedKmH: number): DeadmanStatus {
    ...
    const countdown = Math.max(0, +(this.enforcementTime - this.timer).toFixed(1));
    ...
    return {
      ...
      countdown,
      warningVisual: this.stage === 'visual' || this.stage === 'urgent' || this.stage === 'enforced',
