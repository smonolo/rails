...
export class SceneryManager {
  ...
  private buildCrossingsAndRoads(
    trackNet: TrackNetwork,
    stations: Station[],
    prng: PRNG
  ): void {
    ...
    if (!track0.isClosed) {
      ...
      crossingDistances.push(
        Math.round(track0.totalLength * 0.4),
        Math.round(track0.totalLength * 0.9)
      )
    }
    ...
    for (const targetDist of crossingDistances) {
      ...
      for (const sw of trackNet.switches) {
        if (
          targetDist >= sw.fromDistance - 50 &&
          targetDist <= sw.toDistance + 50
        ) {
          nearSwitch = true
          break
        }
      ...
      const p1 = trackNet.getStaticPointAtDistance(
        1,
        Math.min(targetDist, track1.totalLength)
      )
